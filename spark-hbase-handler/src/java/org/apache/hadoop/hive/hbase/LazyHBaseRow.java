    ...
    if (!fieldsInited[fieldID]) {
      ...
      if (colMap.hbaseRowKey) {
        ...
        // it is a column i.e. a column-family with column-qualifier
          Cell cell =null;
          for (Cell cellTmp : result.rawCells()) {
              if (Arrays.equals(CellUtil.cloneQualifier(cellTmp), colMap.qualifierNameBytes) && Arrays.equals(CellUtil.cloneFamily(cellTmp), colMap.familyNameBytes)) {
                  cell = cellTmp;
                  break;
              }
          }
          bytes = cell == null ? null: CellUtil.cloneValue(cell);
      }
