...
public class HiveHBaseTableInputFormat extends TableInputFormatBase
    ...
    public RecordReader<ImmutableBytesWritable, ResultWritable> getRecordReader(
        ...
        //如果涉及多个key的话，需要整改maping进行scan
        setHbaseKeyTableMaping(jobConf);
        ...
        return new RecordReader<ImmutableBytesWritable, ResultWritable>() {
            ...
            private void setPhoenixHbaseData(ImmutableBytesWritable rowKey, ResultWritable value) throws IOException, InterruptedException {
                String keyMaping = jobConf.get("hbase.table.phoenix.key");
                if (null == keyMaping || keyMaping.isEmpty()) {
                    setUsualPhoenix(rowKey, value);
                } else {
                    setMultiplePhoenix(rowKey, value);
                }
            }

            private void setMultiplePhoenix(ImmutableBytesWritable rowKey, ResultWritable value) throws IOException, InterruptedException {
                String schma = jobConf.get("serialization.ddl");
                String mapping = jobConf.get("hbase.columns.mapping");
                String keyMaping = jobConf.get("hbase.table.phoenix.key");
                Table table = HbaseTableUtil.getTable(schma, mapping, keyMaping);
                byte[] row = recordReader.getCurrentValue().getRow();
                byte[] tmpRow = PhoenixHbaseDataTypeHandle.toHbaseByteMRowKey(row, table.getRowKeyColumn().getDataType());
                if (null == tmpRow) {
                    rowKey.set(row);
                } else {
                    rowKey.set(tmpRow);
                }
                List<Cell> cellList = getCells(rowKey, table);
                //设置多key的数据，为cell
                setMCell(cellList, table, row, rowKey);
                Result resultTmp = Result.create(cellList);
                value.setResult(resultTmp);
            }

            private void setMCell(List<Cell> cellList, Table table, byte[] row, ImmutableBytesWritable rowKey) {
                ...
                for (int i = 0; i < keys.size(); i++) {
                    ...
                    byte[] value_1 = PhoenixHbaseDataTypeHandle.toHbaseByte(row, column.getDataType(), i + 2);
                    Cell cell_1 = CellUtil.createCell(rowKey.get(), Bytes.toBytes(column.getColumnFamily()),
                            Bytes.toBytes(column.getColumnName()),
                            System.currentTimeMillis(), (byte) 4, value_1);
                    cellList.add(cell_1);
                }
            }

            private void setUsualPhoenix(ImmutableBytesWritable rowKey, ResultWritable value) throws IOException, InterruptedException {
                //设置phoenix的数据类型
                String schma = jobConf.get("serialization.ddl");
                String mapping = jobConf.get("hbase.columns.mapping");
                Table table = HbaseTableUtil.getTable(schma, mapping);
                byte[] row = recordReader.getCurrentValue().getRow();
                byte[] tmpRow = PhoenixHbaseDataTypeHandle.toHbaseByte(row, table.getRowKeyColumn().getDataType()); // toHbaseByteRwoKey
                if (null == tmpRow) {
                    rowKey.set(row);
                } else {
                    rowKey.set(tmpRow);
            ...
            private List<Cell> getCells(ImmutableBytesWritable rowKey, Table table) throws IOException, InterruptedException {
                ...
                for (Cell cell : result.rawCells()) {
                    ...
                    //转换phoenix数据类型，避免只有rowkey的情况，会导致查询不出来数据
                    if (Bytes.toString(column_1).equalsIgnoreCase("_0")) {
                        Cell cell_1 = CellUtil.createCell(rowKey.get(), family_1, column_1, System.currentTimeMillis(), cell.getTypeByte(), CellUtil.cloneValue(cell));
                        cellList.add(cell_1);
                        continue;
                    }
                    Column column = table.getColumn(Bytes.toString(column_1));
                    if (column == null) {
                        continue;
                    }
    ...
    private void setHbaseKeyTableMaping(JobConf jobConf) {
        String keyMaping = jobConf.get("hbase.table.phoenix.key");
        if (null != keyMaping && !keyMaping.isEmpty()) {
            StringBuilder stringBuffer = new StringBuilder();
            String mapping = jobConf.get("hbase.columns.mapping");
            String[] colMaping = mapping.split(",");
            for (String column : colMaping) {
                stringBuffer.append(changeColumnKeyMaping(keyMaping, column)).append(",");
            }
            String columnMapingTmp = stringBuffer.toString().substring(0, stringBuffer.length() - 1);
            jobConf.set("hbase.columns.mapping", columnMapingTmp);
        }
    }

    private String changeColumnKeyMaping(String keyMaping, String column) {
        String[] keyMapingArr = keyMaping.split(",");
        for (String key : keyMapingArr) {
            if (column.toLowerCase().contains(key)) {
                if (column.toLowerCase().contains("#b")) {
                    column = ":key#b";
                } else if (column.toLowerCase().contains("#s")) {
                    column = ":key#s";
                } else {
                    column = ":key";
                }
