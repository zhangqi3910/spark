...
import org.apache.hive.storage.jdbc.read.type.JdbcFieldAttr;
...
public class JdbcRecordReader implements RecordReader<LongWritable, ResultWritable> {
    ...
    @Override
    public boolean next(LongWritable key, ResultWritable value) throws IOException {
        try {
            ...
            if (iterator.hasNext()) {
                ...
                if ((record != null) && (!record.isEmpty())) {
                    List<ReadJdbcBean> readJdbcBeans = new ArrayList<>();
                    for (Entry<String, Object> entry : record.entrySet()) {
                        JdbcFieldAttr jdbcFieldAttr = (JdbcFieldAttr) entry.getValue();
                        ReadJdbcBean readJdbcBean = new ReadJdbcBean(entry.getKey(), jdbcFieldAttr.getValue(), jdbcFieldAttr.getJdbcTypeName());
                        readJdbcBeans.add(readJdbcBean);
                    }
                    value.setReadJdbcBeans(readJdbcBeans);
                    return true;
                } else {
